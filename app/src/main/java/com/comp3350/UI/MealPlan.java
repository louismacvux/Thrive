...
public class MealPlan extends AppCompatActivity {
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            switch (position) {
                case 0:
                    break;
                case 1:
                    selected = MealDoc.NonVeg3100;
                    break;
                case 2:
                    selected = MealDoc.NonVeg2600;
                    break;
                case 3:
                    selected = MealDoc.NonVeg2300;
                    break;
                case 4:
                    selected = MealDoc.NonVeg1900;
                    break;
            }
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                switch (position) {
                    case 0:
                        break;
                    case 1:
                        selected = MealDoc.Veg3100;
                        break;
                    case 2:
                        selected = MealDoc.Veg2600;
                        break;
                    case 3:
                        selected = MealDoc.Veg2200;
                        break;
                    case 4:
                        selected = MealDoc.Veg1800;
                }
                String text = parent.getItemAtPosition(position).toString();
                Toast.makeText(parent.getContext(), text, Toast.LENGTH_SHORT).show();
                if (selected != null)
    ...
    public void gotoBMI(View v) {
        Intent intent;
        intent = new Intent(MealPlan.this, WellnessCalculator.class);
    ...
    public void gotoRecipes(View v) {
        Intent pdfViewIntent;
        pdfViewIntent = new Intent(MealPlan.this,ViewPDF.class);
        Bundle bundle = new Bundle();
